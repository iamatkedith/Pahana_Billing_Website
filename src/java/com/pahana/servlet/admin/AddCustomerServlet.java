...
public class AddCustomerServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (userInserted == 0) {
                con.rollback();
                response.sendRedirect(request.getContextPath() + "/admin/customers.jsp?error=Failed to add user!");
                return;
            }
            ...
            if (rs.next()) {
                ...
                con.rollback();
                response.sendRedirect(request.getContextPath() + "/admin/customers.jsp?error=Failed to retrieve user ID!");
                return;
            }
            rs.close();
            psUser.close();
